...
const CATALOG = [
  ...
  { id: 5, name: 'Análisis de laboratorio',    points: 200, emoji: '🔬', description: 'Análisis de rutina con descuento', category: 'diagnostico' },
  { id: 6, name: 'Sesión de kinesiología',     points: 250, emoji: '💆', description: 'Sesión de rehabilitación',     category: 'salud' }
]
...
router.get('/commerces', async (_req, res) => {
  try {
    ...
    console.error('[rewards/commerces]', err.message)
    res.json({
      success: true,
      commerces: [
...
router.post('/redeem-qr', async (req, res) => {
  try {
    ...
    if (!qrCode) return res.status(400).json({ success: false, message: 'QR requerido' })
    ...
    if (error || !data) return res.json({ success: false, message: 'QR no encontrado' })
    if (data.status === 'completed') return res.json({ success: false, message: 'QR ya fue utilizado' })
    if (new Date(data.expires_at) < new Date()) {
      await supabase.from('wr_redemptions').update({ status: 'expired' }).eq('qr_code', qrCode)
